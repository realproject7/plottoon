...
| **PLOT/USD price**                     | `lib/usd-price.getPlotUsdPrice()` (HUNT-backed derivation) | best-effort CoinGecko `token_price/base` against the PLOT token contract in `fetchPlotPrice:` callback — surfaced as `tokenPrice.plotUsd` (#249 Gap D)                                                             | ✓ (best-effort; HUNT-fallback deferred) |
...
### 2. PLOT/USD HUNT-derivation fallback

#249 ships a best-effort CoinGecko `token_price/base` lookup against the PLOT token contract. `tokenPrice.plotUsd` is null when CoinGecko has no coverage, and the PnL row hides gracefully. plotlink-ows uses a HUNT-backed derivation (`lib/usd-price.ts` in that repo) as the canonical fallback; porting it to PlotToon is deferred until CoinGecko coverage is empirically known insufficient.

This is the explicit subject of follow-up issue #264.

### 3. Recovery-attempt history in the Activity feed
...
### 4. On-device screenshot evidence
